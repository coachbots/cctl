time_step = 2
robot_radius = 0.075 #0.065
message_radius = robot_radius*4
import math
import struct
...
def usr(robot):
    ...
    while True:
        ...
        if robot.id == 9:
            ...
            # robot.logger.info("got curr position") 
            # desired_x = -0.1
            # desired_y = -0.4
                
            desired_pose = N_init_positions(robot, robot.id)
            ...
            # desired_theta= math.atan2(own_vopt_y, own_vopt_x)
            ...
            while(robot.get_clock()-last) < 0.3:
                # to_send = package_string([(robot.get_clock()- last), (count%3)])
            ...
            while(time_elapsed) < 0.3:
                ...
                if len(rec_str) > 0:
                    ...
                    # robot.logger.info(count_received)
...
def unpack_string(robot, str):
    ...
    j = 0
    while j < (len(str)):
        # robot.logger.info(str[j])
        ret_temp = str[j].split('\x00')
        # robot.logger.info(ret_temp)
        ret = ret_temp[0].split(',')        
        str_len = int(ret[0])            
        for i in range(str_len):
        # print(ret[i+1])
            string_return.append(float(ret[i+1]))
...
def turn_to_angle_old(robot, curr_theta, desired_theta):
    import math
